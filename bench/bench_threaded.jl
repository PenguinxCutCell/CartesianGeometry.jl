...
# Helper to create 4D mesh  
function create_mesh_4d(n)
    x = collect(range(0.0, 1.0, length=n+1))
    y = collect(range(0.0, 1.0, length=n+1))
    z = collect(range(0.0, 1.0, length=n+1))
    w = collect(range(0.0, 1.0, length=n+1))
    return (x, y, z, w)
end
...
levelset_4d = HyperSphere(R, (0.5, 0.5, 0.5, 0.5))
...
function run_benchmark()
    ...
    integrate(Tuple{0}, levelset_4d, mesh_4d, T, zero; method=:vofijul)
    integrate_threaded(Tuple{0}, levelset_4d, mesh_4d, T, zero; method=:vofijul)
    ...
    println()
    println("-"^70)
    println("4D Volume Integration (Tuple{0})")
    println("-"^70)
    println(@sprintf("%-12s %12s %12s %12s %12s", "Resolution", "Serial (s)", "Threaded (s)", "Speedup", "Cells"))

    resolutions_4d = [5, 7, 9, 11, 13]

    for n in resolutions_4d
        mesh = create_mesh_4d(n)
        num_cells = n * n * n * n

        # Serial timing
        GC.gc()
        t_serial = @elapsed integrate(Tuple{0}, levelset_4d, mesh, T, zero; method=:vofijul)

        # Threaded timing
        GC.gc()
        t_threaded = @elapsed integrate_threaded(Tuple{0}, levelset_4d, mesh, T, zero; method=:vofijul)

        speedup = t_serial / t_threaded
        println(@sprintf("%-12s %12.4f %12.4f %12.2fx %12d", "$(n)×$(n)×$(n)×$(n)", t_serial, t_threaded, speedup, num_cells))
    end
